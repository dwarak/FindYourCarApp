	...
	public function carFilterAction()
		...
		if($Zipcode ==""){
			...
			function getLnt($zip){
				...
				$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($zip)."&key=AIzaSyBE4mCmtvEOFybBD3UQlGF9CF7AdA77lrA";
				$result_string = file_get_contents($url);
				$result = json_decode($result_string, true);
				$result1[]=$result['results'][0];
				$result2[]=$result1[0]['geometry'];
...
if ($err) {
  ...
  echo $response;
		...
		public function zipcompareAction(){
			if (!$this->getRequest()->isPost())
			{
				$encode = json_encode(array('error'=>"This function accepts only POST."));
				$this->header->setHttpResponseCode ( 405 );
			...
			$userzip = trim ( $this->getRequest ()->getParam ( 'userzip' ) );
			$dealerzip = trim ( $this->getRequest ()->getParam ( 'dealerzip' ) );
			//$userzip = "94536";
			//$dealerzip = "78233";
			...
			curl_setopt_array($curl, array(
			CURLOPT_URL => "http://maps.googleapis.com/maps/api/distancematrix/json?origins=".$userzip."&destinations=".$dealerzip."&mode=bicycling&sensor=false",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 30,
			...
			CURLOPT_HTTPHEADER => array( 
			"Cache-Control: no-cache",
			"Postman-Token: 3d8884d6-8e57-a4ef-01cd-9bdd941433cf"
			),
		));
		...
		if ($err) {
		  echo "cURL Error #:" . $err;
			...
			if($key=='0'){
				foreach($value as $key1=>$listdata){
					if($key1=='elements'){
						foreach($listdata as $finaldata){
							$valuedata = $finaldata['distance'];
						}
					}
				}
			}
	...
	public function zipcompareDistanceAction(){
		if (!$this->getRequest()->isPost())
			{
				$encode = json_encode(array('error'=>"This function accepts only POST."));
				$this->header->setHttpResponseCode ( 405 );
			...
			$Zipcode = trim ( $this->getRequest ()->getParam ( 'Zipcode' ) );
			$latitudeTo = trim ( $this->getRequest ()->getParam ( 'latitudeTo' ) );
			$longitudeTo = trim ( $this->getRequest ()->getParam ( 'longitudeTo' ) );
			
			//$Zipcode = "94536";
			//$latitudeTo = '37.3598283';
			//$longitudeTo = '-121.98143540000001';
			...
			$result1[]=$datalist['results'][0];
				...
				$valueslist = $result3[0];
				$latitudeFrom  =  $valueslist['lat'];
				$longitudeFrom = $valueslist['lng'];
				
				/* $latitudeFrom = '37.5641425';
				$longitudeFrom = '-122.00417900000002'; */
				...
				$final['Distance'] = ($distance *0.621);
				echo json_encode($final);
	}
		
	public function vinIotAction(){
		 if (!$this->getRequest()->isPost())
			{
				$encode = json_encode(array('error'=>"This function accepts only POST."));
				$this->header->setHttpResponseCode ( 405 );
			...
			//$vin = "3C63RRLL3JG164277";
			
			$dataview = $this->Showroom->iotvindata($vin);
